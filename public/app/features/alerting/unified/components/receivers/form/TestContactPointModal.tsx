...
export const TestContactPointModal = ({ isOpen, onDismiss, onTest }: Props) => {
  ...
  return (
    <Modal
      onDismiss={onDismiss}
      isOpen={isOpen}
      title={t('ablestack-wall.alert.test-contact-point', 'Test contact point')}
    >
      <div className={styles.section}>
        <Label>{t('ablestack-wall.alert.notification-message', 'Notification message')}</Label>
        <RadioButtonGroup
      ...
      <FormProvider {...formMethods}>
        <form onSubmit={formMethods.handleSubmit(onSubmit)}>
          ...
          <Modal.ButtonRow>
            <Button type="submit">{t('ablestack-wall.alert.send-test-notification', 'Send test notification')}</Button>
          </Modal.ButtonRow>
        </form>
      </FormProvider>
    </Modal>
